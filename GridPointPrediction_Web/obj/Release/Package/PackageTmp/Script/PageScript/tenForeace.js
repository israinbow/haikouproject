...
selectData = function (value) {
    ...
    if (value == 06) {
        ...
        $("#nameil").text("今天早晨至傍晚（6-20时）天气预测    (数据来源：格点预报，仅供预报员参考)");
...
//十天预报上半部分
doload = function () {
    $.post("/WebService.asmx/getTideHigh", { dateTime: time }, function (msg) {
    ...
    //下半部分预报员数据查询
    $.post("/WebService.asmx/GetWelfareForecastDaysInfo", { dateTime: time }, function (result) {
        ...
        if (result != "" && result != undefined) {
            ...
            for (var i = 0; i < result.length; i++) {
                ...
                //  var d = eval('new Date( ' + strdate.substr(1, strdate.length - 2) + ')');
               // var d = new Date(strdate.substr(0, strdate.length));
    ...
    //获取7天格点数据参考
    $.post("/WebService.asmx/GetWelfareForecastDaysECInfo", { dateTime: time }, function (result) {
        if (result != "" && result != undefined) {
            result = JSON.parse(result);
    ...
    $.post("/WebService.asmx/Get10DaysForecaster", { dateTime: time }, function (result) {
        if (result != "" && result != undefined) {
            result = JSON.parse(result);
        ...
        $.post("/WebService.asmx/GetUserInfoname", {}, function (result) {
            for (var i = 0; i < result.length; i++) {
                ...
                if (result[i].UserName == user && result[i].UPassword == $("#passwad").val()) {
                    ...
                    if (document.getElementById("chuanzhen").checked) { check[0] = true; }
                    else { check[0] = false; }
                    if (document.getElementById("weibo").checked) { check[1] = true; }
                    else { check[1] = false; }
                    if (document.getElementById("word").checked) { check[2] = true; }
                    else { check[2] = false; }
                    if (document.getElementById("realsofa").checked) { check[3] = true; }
                    else { check[3] = false; }
                    $.post("/WebService.asmx/SaveDate", { todayData: todayforecast, Arain: RainEourpe, AimageUrl: imageUrl, AChEnEdit: ChEnEdit, ATempEdit: TempEdit, AWindEdit: WindEdit, AHumitEdit: HumitEdit, Stat: check }, function (result) {

                    });

                    // window.location.href = "yula.aspx?id=" + weathertime + "&user=" + $("#cbUser").val();
                    break;
                }
                else {
                    layer.msg("请输入正确密码");
...
GetluruResult = function () {
   ...
   var times = $("#date").val() + " " + Tvalue + ":00:00"; //zhou 添加
    //WebMethod.callBack("GetweatherResult", { weather: time }, function (msg) {
...
ReadValueForease = function () {
    $.post("/WebService.asmx/GetWelfareForecastCapsInfo", { dateTime: weathertime }, function (msg) {
        if (msg != "" && msg != undefined) {
            try {
                $("#weatherValue").html(msg.PastTimes);
                $("#weatherFuture").html(msg.Future);
                $("#mintemp").val(msg.MinTemp);
                $("#maxtemp").val(msg.MaxTemp);
                $("#minHumidity").val(msg.Humidity);
